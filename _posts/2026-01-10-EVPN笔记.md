...
简单来说，ESI Filter Label 是 EVPN 在 MPLS 数据平面上实现**水平分割（Split Horizon）**机制的关键手段，目的是为了防止在多归属站点出现环路。
...
在传统的二层交换机网络中，如果你收到一个 ARP 广播请求，或者一个目的 MAC 地址未知的帧，交换机会怎么做？它会简单粗暴地把它从**同一个 VLAN 内的所有其他接口泛洪（Flood）出去**。
...
- **范围：** Type 3 路由是**基于每个广播域（BD/EVI）**通告的。如果一个 PE 上有 10 个 BD，它就会通告 10 条 Type 3 路由。
...
4. PMSI Tunnel Attribute (最关键的属性)：
   ...
   - **MPLS Label / VNI (下游分配的 BUM 标签)：** 这是一个非常重要的细节。发送端 PE 会在 BGP 中告诉别人一个**特定的 MPLS 标签（在 MPLS EVPN 中）**。
     - *注意区别：* Type 2 路由携带的是用于单播的标签；Type 3 路由携带的是**专门用于 BUM 流量的标签**。
...
![image-20260110112359028](https://raw.githubusercontent.com/LiuMaoDou/Files/main/Pics/image-20260110112359028.png)
...
![Gemini_Generated_Image_1ef1ia1ef1ia1ef1](https://raw.githubusercontent.com/LiuMaoDou/Files/main/Pics/Gemini_Generated_Image_1ef1ia1ef1ia1ef1.png)
...
![Gemini_Generated_Image_hmeiushmeiushmei](https://raw.githubusercontent.com/LiuMaoDou/Files/main/Pics/Gemini_Generated_Image_hmeiushmeiushmei.png)
